...
OUTPUT_FILE="${APP_DIR}/sealed_secret_healthchecks.yaml"
NAMESPACE="monitoring"
SECRET_NAME="alert-webhook-healthchecks"
...
echo "alert-webhook-healthchecks SealedSecret generator"
echo "=================================================="
echo ""
echo "Podaj READ-ONLY klucz API projektu healthchecks (hcr_…) —"
echo "hc.ugard.win -> Settings -> API Access. Używany przez komendę"
echo "„backups\" w alert-webhooku (tylko odczyt listy checków)."
echo ""

HC_API_KEY=""
while [[ -z "$HC_API_KEY" ]]; do
    read -r -s -p "HEALTHCHECKS_API_KEY: " HC_API_KEY
    echo
    HC_API_KEY="${HC_API_KEY#"${HC_API_KEY%%[![:space:]]*}"}"
    HC_API_KEY="${HC_API_KEY%"${HC_API_KEY##*[![:space:]]}"}"
    [[ -z "$HC_API_KEY" ]] && echo "Value cannot be empty."
done
...
kubectl "${KUBECTL_ARGS[@]}" create secret generic "$SECRET_NAME" \
    --namespace "$NAMESPACE" \
    --from-literal=HEALTHCHECKS_API_KEY="$HC_API_KEY" \
    --dry-run=client \
    -o json \
    | kubeseal \
        --format yaml \
...
if ! grep -q "sealed_secret_healthchecks.yaml" "$KUSTOMIZATION_FILE"; then
    echo "  - sealed_secret_healthchecks.yaml" >> "$KUSTOMIZATION_FILE"
fi
